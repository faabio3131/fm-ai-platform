...
| WP-014 | Engenharia de Cardápio + Ficha | GAP HTTP/WEB | Ficha manual migrada; importação IA sem boundary reutilizável | legacy_cardapio_transacoes + lógica no app.py | `/admin/catalogo` | Aguardar autorização para extrair boundary da importação IA | missão atual | EM MIGRAÇÃO | Tentativa Codex descartada; não duplicar lógica no HTTP |
| WP-015 | Estoque / Almoxarifado / Validades | GAP HTTP/WEB | Gestão manual/lote migrada; IA/forecast sem boundary reutilizável | core/estoque + legado escopado + lógica no app.py | `/admin/estoque` | Aguardar autorização para extrair boundary de IA/forecast | missão atual | EM MIGRAÇÃO | Sem lote físico ou FEFO inventado |

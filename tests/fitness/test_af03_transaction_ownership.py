...
def test_af03_t_legacy_cliente_e2e_ui_has_no_transaction_owner() -> None:
    offenders = [
        record
        for record in _scan_transaction_calls()
        if (
...
def test_af03_y_pagbank_webhook_http_has_no_transaction_owner() -> None:
    offenders = [
        record
        for record in _scan_transaction_calls()
        if (
            record["path"] == "http_api/app.py"
            and record["function"]
            == "webhook_pagbank"
        )
    ]

...
const tmpDir = process.env.FM_AI_TEST_TMPDIR
  ...
  : resolve(root, '.tmp', 'fm-ai-playwright', 'entrega');
const port = process.env.FM_AI_E2E_PORT || '8525';
...
const app = resolve(__dirname, 'app_entrega.py');
const child = spawn(
  python,
  [
    '-m',
...
child.on('error', error => {
  console.error(`[fm-ai-entrega-e2e] Falha ao iniciar ${python}:`, error);
  process.exit(1);
});
child.on('exit', (code, signal) => {
  console.log(`[fm-ai-entrega-e2e] Streamlit encerrou (code=${code}, signal=${signal}).`);

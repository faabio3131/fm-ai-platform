...
import {
  ...
  despacharResgate,
  listarClientesCrm,
  listarResgatesInativos,
  type CashbackDetalhe,
  type CrmCliente,
  type ResgateClienteInativo,
...
export function CrmWorkspace() {
  ...
  const [resgates, setResgates] = useState<ResgateClienteInativo[]>([]);
  ...
  const [resgateEmEnvio, setResgateEmEnvio] = useState<number | null>(null);
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      setResgates(oportunidades.itens);
  ...
  async function enviarResgate(oportunidade: ResgateClienteInativo) {
    if (resgateEmEnvio !== null) return;
    setResgateEmEnvio(oportunidade.legacy_cliente_id);
    setErro(null);
    setAviso(null);
    try {
      const resultado = await despacharResgate(
        oportunidade.legacy_cliente_id,
        oportunidade.mensagem_sugerida,
      );
      if (resultado.enviado) {
        setAviso(`Campanha de resgate enviada para ${oportunidade.nome}.`);
      } else {
        setErro(`Envio não realizado: ${resultado.motivo}.`);
      }
    } catch (error) {
      setErro(mensagemErro(error));
    } finally {
      setResgateEmEnvio(null);
    }
  }
      ...
      <div className="mx-auto max-w-[1500px] space-y-6">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Button
            ...
            disabled={loading || busy || resgateEmEnvio !== null}
          >
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60">
          <div className="flex items-center gap-2 border-b border-slate-800 px-5 py-4">
            ...
            <div>
              <h2 className="font-bold">Marketing · Resgate de clientes inativos</h2>
              <p className="mt-1 text-xs text-slate-500">
                Oportunidades e mensagens produzidas pela autoridade de CRM para a unidade ativa.
              </p>
            </div>
          </div>
          {loading ? (
            <div className="flex min-h-36 items-center justify-center gap-2 text-sm text-slate-400">
              <LoaderCircle className="size-5 animate-spin" /> Carregando oportunidades…
            </div>
          ) : resgates.length === 0 ? (
            <p className="p-5 text-sm text-slate-400">
              Nenhum cliente elegível para resgate nesta unidade.
            </p>
          ) : (
            <div className="grid gap-4 p-5 lg:grid-cols-2">
              {resgates.map((oportunidade) => (
                <article
                  key={oportunidade.cliente_id}
                  className="rounded-xl border border-slate-800 bg-slate-950/70 p-4"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="truncate font-semibold text-white">{oportunidade.nome}</p>
                      <p className="mt-1 truncate text-xs text-slate-500">
                        {oportunidade.whatsapp} · {oportunidade.status}
                      </p>
                    </div>
                    <p className="shrink-0 text-sm font-bold text-emerald-300">
                      {moeda(oportunidade.total_gasto)}
                    </p>
                  </div>
                  <p className="mt-3 text-xs text-slate-500">
                    Última compra: {dataHora(oportunidade.ultima_compra)}
                  </p>
                  <p className="mt-3 rounded-lg border border-violet-500/20 bg-violet-500/10 p-3 text-sm text-violet-100">
                    {oportunidade.mensagem_sugerida}
                  </p>
                  <Button
                    type="button"
                    className="mt-4 w-full"
                    disabled={resgateEmEnvio !== null}
                    onClick={() => void enviarResgate(oportunidade)}
                  >
                    {resgateEmEnvio === oportunidade.legacy_cliente_id ? (

...
from .modelos import ChecklistExpedicao, Entrega, ProvaEntrega, StatusEntrega, TentativaEntrega
...
class ServicoEntrega:
    """Máquina logística com CAS, idempotência, RBAC e limites financeiros."""
    ...
    def marcar_pedido_pronto(
        ...
        *,
        ...
        if atual.status not in {StatusEntrega.AGUARDANDO_PRODUCAO, StatusEntrega.ATRIBUIDA}:
    ...
    def concluir_checklist(
        ...
        *,
        ...
        if atual.status not in {StatusEntrega.AGUARDANDO_EXPEDICAO, StatusEntrega.ATRIBUIDA}:
            raise ErroEntrega("transicao_entrega_invalida")
        instante = self._agora()
        novo_status = (
    ...
    def confirmar_entrega(
        ...
        *,
        ...
        if not self.financeiro_resolvido(
            atual.tenant_id, atual.unidade_id, atual.pedido_id
        ):
    ...
    def cancelar(
        ...
        *,
        ...
        if atual.status not in permitidos:
            raise ErroEntrega("transicao_entrega_invalida")
        if not self.pedido_cancelado(atual.tenant_id, atual.unidade_id, atual.pedido_id):
    ...
    def _repeticao(
        ...
        evento = self.repositorio.buscar_evento_idempotente(
            contexto.tenant_id, contexto.unidade_id, chave
        )
        if evento is None:
            return None
        ...
        entrega = self.repositorio.buscar(
            contexto.tenant_id, contexto.unidade_id, evento.entrega_id
        )
        if entrega is None:
    ...
    def _obter(self, entrega_id: str, contexto: ContextoExecucao) -> Entrega:
        entrega = self.repositorio.buscar(
            contexto.tenant_id, contexto.unidade_id, entrega_id
        )
        if entrega is None:
    ...
    def _validar_escopo(entrega: Entrega, contexto: ContextoExecucao) -> None:
        if entrega.tenant_id != contexto.tenant_id or entrega.unidade_id != contexto.unidade_id:

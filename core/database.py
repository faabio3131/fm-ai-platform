...
Base.metadata.create_all(
    bind=engine
)  # Garante schema relacional atualizado
...
# ==============================================================================
# ABA 3: FRENTE DE CAIXA (PDV) COM SESSÃO ATIVA GARANTIDA (FASE 2)
# ==============================================================================
with aba3:
    ...
    if not lista_pratos:
        ...
        with col_pdv1:
            ...
            # --- BOTÃO COM SESSÃO FRESCA DO ORM ---
            if st.button(
                ...
                try:
                    # Busca o produto com uma sessão aberta e ativa
                    prod_atual = (
                    ...
                    if not prod_atual:
                        st.error(
                            "❌ Produto não encontrado ou removido do cardápio."
                        )
                    else:
                        # 1. Registrar a venda financeira
                        nova_venda = Venda(
                        ...
                        # 2. Baixar estoque pela Ficha Técnica (Receita)
                        relatorio_baixa = []
                        ...
                        if prod_atual.ingredientes:
                            ...
                            st.warning(
                                "⚠️ Este produto não possui Ficha Técnica vinculada. A venda foi registrada, mas o estoque não foi alterado por gramagem."
                            )

                        db_acao.commit()

                        st.success(
                            f"🎉 Pedido de **{quantidade}x {prod_atual.nome}** registrado com sucesso!"
                        )

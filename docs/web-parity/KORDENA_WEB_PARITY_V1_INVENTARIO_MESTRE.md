...
| WP-023 | Usuários / Papéis / Permissões | Application e UI originais localizados; reprodução comprovou ampliação indevida de membership pela unidade padrão; **CORREÇÃO APLICADA**: invariante `unidade_padrao_id ∈ unidades_permitidas_validadas` em `application/administracao_proprietario.py:528-529,587-588` (SHA `27dc56e`); **MIGRAÇÃO WEB CONCLUÍDA**: rota `/admin/usuarios`, HTTP `/v1/admin/usuarios`, shell module `usuarios`, RBAC `admin.acessar` + `usuario.gerenciar` (SHA `ebe8633`) | Implementado; correção de autoridade publicada | `/admin/usuarios` + HTTP `/v1/admin/usuarios` | MIGRADO | `27dc56e` (fix), `ebe8633` (web) / PR #118 | MIGRADO | Bloqueio removido; Core/Infra intactos; RBAC `admin.acessar` + `usuario.gerenciar` + `permissao.gerenciar` para sensíveis |
...
- **09/09/2026 - Ciclo WP-023 + WP-024 — Bloco A (correção WP-023) concluído:** pré-flight confirmado a partir do baseline `0b4537e29e0b9a19fe6bc6e676350cd824124ae4`, PR #118 OPEN/DRAFT e árvore limpa. Correção cirúrgica do invariante `unidade_padrao_id ∈ unidades_permitidas_validadas` aplicada em `application/administracao_proprietario.py` nas linhas 528-529 (`criar_usuario`) e 587-588 (`atualizar_usuario`). Testes validam: unidade única válida, múltiplas unidades válidas, unidade padrão no conjunto permitido, unidade padrão fora do conjunto explícito (rejeitada), unidade padrão de tenant não autorizável (rejeitada), rejeição sem criar usuário/membership/auditoria parcial, comportamento legítimo preservado. Cobertura equivalente em `atualizar_usuario`. Ruff e testes de integração administrativos verdes. Commit `27dc56e0ea4dccf1e4ff6471a5f87e4f70f02bbc` publicado e confirmado remoto (local HEAD == remote HEAD). Documentação `WP023_WP024_CICLO_2X2.md` atualizada. Bloqueio removido; Core/Infra intactos. WP-023 desbloqueado para migração Web; WP-024 aguarda.
- **09/09/2026 - Ciclo WP-023 + WP-024 — Bloco B (WP-023 Web) concluído:** migração Web completa de Usuários/RBAC. Rota `/admin/usuarios` criada com HTTP `/v1/admin/usuarios` (GET/POST/PUT), shell module `usuarios` com ícone `Users`, RBAC `admin.acessar` + `usuario.gerenciar` (step-up para sensíveis via `permissao.gerenciar`). Reutilização total da `AplicacaoAdministracaoProprietarioV1` corrigida. Arquivos: `http_api/admin_usuarios.py`, `web/src/app/admin/usuarios/page.tsx`, `web/src/features/backoffice/usuarios/components/UsuariosWorkspace.tsx`, `module-registry.ts`, `DashboardHome.tsx`, `UnifiedAppShell.tsx`. Testes: 24 testes Python passam (integração + contrato HTTP), TypeScript typecheck verde. Commit `ebe863311775feb158a450ef60ee122810857e90` publicado e confirmado remoto. WP-023 promovido para MIGRADO no inventário. Iniciado Bloco C (WP-024).

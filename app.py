...
with aba6:
    ...
    st.markdown(
        "Simule o atendimento ao cliente via WhatsApp. A **Mica I.A.** entende texto, áudio e fotos, faz cross-selling dinâmico (upsell) e gera cobrança Pix nativa com baixa automática de estoque."
    )
    ...
    try:
        ...
        if produtos_bot:
            lista_menu = [
                f"- {p.nome}: R$ {p.preco_venda:.2f} ({p.categoria})"
                for p in produtos_bot
            ]
            menu_disponivel_bot = "\n".join(lista_menu)
        else:
    ...
    with col_bot_1:
        ...
        telefone_cliente_bot = st.text_input(
            "📱 WhatsApp do Cliente", value="5511999995432", key="tel_bot"
        )
        foto_pedido_bot = st.file_uploader(
            "📸 Foto de referência ou áudio (Opcional - Multimodal)",
            type=["jpg", "png", "ogg", "mp3"],
        )

    with col_bot_2:
        st.subheader("💬 Mensagem do Cliente")
        mensagem_cliente_bot = st.text_area(
            "Digite o que o cliente enviou no WhatsApp:", height=130, key="msg_bot"
        )
    ...
    btn_acionar_mica = st.button(
        "🚀 Processar Pedido & Atendimento com a Mica I.A.", use_container_width=True
    )

if btn_acionar_mica:
    if not telefone_cliente_bot or (not mensagem_cliente_bot and not foto_pedido_bot):
        st.error(
            "⚠️ Por favor, informe o WhatsApp do cliente e digite a mensagem ou envie o áudio/foto do pedido!"
        )
    else:
        ...
        texto_base_cliente = (
            mensagem_cliente_bot
            if mensagem_cliente_bot
            else "[Cliente enviou um áudio ou arquivo de pedido]"
        )

        with st.spinner(
            "🤖 A assistente virtual Mica está interpretando a mensagem/áudio, calculando o pedido e gerando o Pix..."
        ):
            try:
                prompt_mica = f"""
                ...
                if (
                    "cartao" in resp_zap_lower
                    or "cartão" in resp_zap_lower
                    or "credito" in resp_zap_lower
                    or "crédito" in resp_zap_lower
                ):
                ...
                st.warning(
                    f"⚠️ Aviso da I.A. ({e_ia}). Ativando modo de Atendimento Comercial Automático de Segurança."
                )

                msg_lower = texto_base_cliente.lower()
                if (
                    "credito" in msg_lower
                    or "crédito" in msg_lower
                    or "cartao" in msg_lower
                    or "cartão" in msg_lower
                ):
                    forma_pag_texto = "Cartão de Crédito"
                ...
                dados_pedido_mica = {
                    ...
                    "itens": [
                        {"nome_produto": "Mica Royal Truffle Bacon", "quantidade": 1}
                    ],
        ...
        try:
            cli_db_mica = (
                db_exec_mica.query(Cliente)
                .filter(Cliente.whatsapp == telefone_cliente_bot)
                .first()
            )
            if not cli_db_mica:
                cli_db_mica = Cliente(
                    nome="Cliente WhatsApp (Mica)",
                    whatsapp=telefone_cliente_bot,
                    status="Ativo",
                    saldo_cashback=0.0,
                )
            ...
            for item_m in itens_comprados_mica:
                ...
                prod_db_m = (
                    db_exec_mica.query(Produto)
                    .filter(Produto.nome.ilike(f"%{nome_p_mica}%"))
                    .first()
                )
                if not prod_db_m:
                ...
                if prod_db_m:
                    ...
                    db_exec_mica.add(
                        Venda(
                            ...
                            forma_pagamento=forma_pag_texto,
            ...
            if prod_db_m:
                itens_ficha = (
                    db_exec_mica.query(FichaTecnica)
                    .filter_by(produto_id=prod_db_m.id)
                    .all()
                )
                for ficha in itens_ficha:
                    insumo = (
                        db_exec_mica.query(Insumo).filter_by(id=ficha.insumo_id).first()
                    )
                    if insumo:
            ...
            db_exec_mica.commit()
            st.success(
                f"🚀 Venda integrada no PDV ({forma_pag_texto}) e estoque baixado com sucesso!"
            )
